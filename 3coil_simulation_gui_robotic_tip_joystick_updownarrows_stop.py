...
def simulate_coil(solenoid_current=15):
    ...
    def stop_callback(state):
        if state:  # If button is clicked (ON)
            emergency_stop()
            pl.add_text("🚨 EMERGENCY STOPPED!", position="upper_left", font_size=20, color="red")
            time.sleep(1)
            pl.close()
    ...
    def move_down(_):
     ...
     # UP /DOWN buttons
    pl.add_sphere_widget(move_up, center=button_up_pos, radius=0.015, color="blue")
    pl.add_sphere_widget(move_down, center=button_down_pos, radius=0.015, color="blue")
